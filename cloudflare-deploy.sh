...
if [[ -t 1 ]] && command -v tput &>/dev/null && [[ "$(tput colors 2>/dev/null || echo 0)" -ge 8 ]]; then
	...
	BOLD="" RESET="" RED="" GREEN="" YELLOW="" BLUE="" CYAN="" WHITE="" DIM=""
fi
...
preflight_checks() {
	...
	if [[ ! -d "node_modules" ]]; then
		print_warn "node_modules not found. Running 'pnpm install' first..."
		pnpm install
	...
	print_ok "All required tools found (git, node, pnpm)."

	# 5. Check for unstaged changes to tracked files (just warn, not abort)
	print_step "Checking working tree..."
	if git diff --quiet && git diff --cached --quiet; then
		print_warn "Working tree is clean. Deploying without new commit."
	else
		local changed
...
run_quality_check() {
	print_header "STAGE 1 / 4 — QUALITY GATE (pnpm run check)"
...
run_build() {
	print_header "STAGE 2 / 4 — PRODUCTION BUILD (pnpm run build)"
...
run_commit() {
	...
	local commit_msg="deploy: auto-deploy from ${CURRENT_BRANCH} @ ${timestamp} [${changed_files}]"
	if [[ ${#commit_msg} -gt 72 ]]; then
		commit_msg="deploy: auto-deploy from ${CURRENT_BRANCH} @ ${timestamp}"
	fi
...
run_push() {
	print_header "STAGE 4 / 4 — LOCAL DEPLOYMENT TO CLOUDFLARE PAGES"
	...
	print_step "Uploading build artifacts to Cloudflare Pages via Wrangler..."

	if npx wrangler pages deploy out --project-name portfolio --branch "$CURRENT_BRANCH" --commit-dirty=true; then
		echo ""
		print_ok "Successfully deployed build to ${BOLD}Cloudflare Pages${RESET}."
	else
		print_error "Failed to push to Cloudflare Pages. Check your 'wrangler login' or project settings."
		exit 1
	fi

	# 3. Push the source code to GitHub as well (to keep repo in sync)
	echo ""
	print_step "Syncing source code to GitHub main branch..."
	if [[ "${COMMIT_SKIPPED:-false}" == "false" ]]; then
		git push origin "$CURRENT_BRANCH"
		print_ok "Source code synced to GitHub."
	else
		print_info "Source code already in sync on GitHub."
	fi
}
...
print_summary() {
	...
	echo ""
	if [[ "${COMMIT_SKIPPED:-false}" == "false" ]]; then
		print_info "Code synced to GitHub. Build deployed to Cloudflare Pages."
	else
		print_info "Code unchanged. Build re-deployed to Cloudflare Pages."
	fi
	echo ""
}
